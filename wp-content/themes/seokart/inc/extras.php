 ...
 * @return array
 */
function seokart_body_classes( $classes ) {
...
add_filter( 'body_class', 'seokart_body_classes' );
...
if (!function_exists('seokart_str_replace_assoc')) {

    /**
     * seokart_str_replace_assoc
     * @param  array $replace
     * @param  array $subject
     * @return array
     */
    function seokart_str_replace_assoc(array $replace, $subject) {
 ...
 */
function seokart_add_to_cart_fragment( $fragments ) {
...
add_filter( 'woocommerce_add_to_cart_fragments', 'seokart_add_to_cart_fragment' );
...
if ( ! function_exists( 'seokart_breadcrumb_title' ) ) {
	function seokart_breadcrumb_title() {
		...
		elseif ( is_day() ) : 
			...
			printf( __( 'Daily Archives: %s', 'seokart' ), get_the_date() );
		
		elseif ( is_month() ) :
		
			printf( __( 'Monthly Archives: %s', 'seokart' ), (get_the_date( 'F Y' ) ));
			
		elseif ( is_year() ) :
		
			printf( __( 'Yearly Archives: %s', 'seokart' ), (get_the_date( 'Y' ) ) );
			
		elseif ( is_category() ) :
		
			printf( __( 'Category Archives: %s', 'seokart' ), (single_cat_title( '', false ) ));

		elseif ( is_tag() ) :
		
			printf( __( 'Tag Archives: %s', 'seokart' ), (single_tag_title( '', false ) ));
			
		elseif ( is_404() ) :

			printf( __( 'Error 404', 'seokart' ));
			
		elseif ( is_author() ) :
		
			printf( __( 'Author: %s', 'seokart' ), (get_the_author( '', false ) ));		
...
function seokart_breadcrumbs() {
	
	$showOnHome	= esc_html__('1','seokart'); 	// 1 - Show breadcrumbs on the homepage, 0 - don't show
	$delimiter 	= '';   // Delimiter between breadcrumb
	$home 		= esc_html__('Home','seokart'); 	// Text for the 'Home' link
	$showCurrent= esc_html__('1','seokart'); // Current post/page title in breadcrumb in use 1, Use 0 for don't show
    ...
    if ( is_category() ) 
	{
		...
		echo $before . esc_html__('Archive by category','seokart').' "' . esc_html(single_cat_title('', false)) . '"' .$after;
		
	} 
	
	elseif ( is_search() ) 
	{
		echo $before . esc_html__('Search results for ','seokart').' "' . esc_html(get_search_query()) . '"' . $after;
	} 
	...
	elseif ( is_tag() ) 
	{
		echo $before . esc_html__('Posts tagged ','seokart').' "' . esc_html(single_tag_title('', false)) . '"' . $after;
	} 
	
	elseif ( is_author() ) {
		global $author;
		$userdata = get_userdata($author);
		echo $before . esc_html__('Articles posted by ','seokart').'' . $userdata->display_name . $after;
	} 
	
	elseif ( is_404() ) {
		echo $before . esc_html__('Error 404 ','seokart'). $after;
    }
	
    if ( get_query_var('paged') ) {
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
		echo ' ( ' . esc_html__('Page','seokart') . '' . esc_html(get_query_var('paged')). ' )';
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
    }
...
add_action( 'wp_ajax_seokart_dismissed_notice_handler', 'seokart_ajax_notice_handler' );

/**
 * AJAX handler to store the state of dismissible notices.
 */
function seokart_ajax_notice_handler() {
    if ( isset( $_POST['type'] ) ) {
...
function seokart_deprecated_hook_admin_notice() {
        ...
        if ( ! get_option('dismissed-get_started', FALSE ) ) {
            ...
            <div class="updated notice notice-get-started-class is-dismissible" data-notice="get_started">
                <div class="seokart-getting-started-notice clearfix">
                    <div class="seokart-theme-screenshot">
                        <img src="<?php echo esc_url( get_stylesheet_directory_uri() ); ?>/screenshot.jpg" class="screenshot" alt="<?php esc_attr_e( 'Theme Screenshot', 'seokart' ); ?>" />
                    </div><!-- /.seokart-theme-screenshot -->
                    <div class="seokart-theme-notice-content">
                        <h2 class="seokart-notice-h2">
                            <?php
                        printf(
                        /* translators: 1: welcome page link starting html tag, 2: welcome page link ending html tag. */
                            esc_html__( 'Welcome! Thank you for choosing %1$s!', 'seokart' ), '<strong>'. wp_get_theme()->get('Name'). '</strong>' );
                        ?>
                        </h2>

                        <p class="plugin-install-notice"><?php echo sprintf(__('Install and activate <strong>Burger Companion</strong> plugin for taking full advantage of all the features this theme has to offer.', 'seokart')) ?></p>

                        <a class="seokart-btn-get-started button button-primary button-hero seokart-button-padding" href="#" data-name="" data-slug=""><?php esc_html_e( 'Get started with SeoKart', 'seokart' ) ?></a><span class="seokart-push-down">
                        <?php
                        ...
                        ?>
                    </div><!-- /.seokart-theme-notice-content -->
                </div>
            </div>
        <?php }
}

add_action( 'admin_notices', 'seokart_deprecated_hook_admin_notice' );

/*******************************************************************************
 *  Plugin Installer
 *******************************************************************************/

add_action( 'wp_ajax_install_act_plugin', 'seokart_admin_install_plugin' );

function seokart_admin_install_plugin() {
    /**
     * Install Plugin.
     */
